...
class LogisticResidualAbsProcessor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        radius = F.elu(self.fc_radius(x)) + 1 + self.positive_lower_bound

        source_amplitude = (
            F.elu(self.fc_source_amplitude(x)) + 1 + self.positive_lower_bound
        )
        background_ratio = F.sigmoid(self.fc_background_ratio(x))
...
class LogisticUnetProcessor(nn.Module):
    def __init__(
        ...
        self.unet = Unet(in_chans=2, out_chans=1, chans=chans, num_pool_layers=pools)
        self.logistic = LogisticResidualAbsProcessor(
            hidden_size, positive_lower_bound, decay_upper_bound
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rot = torch.cat(
            (
                torch.tanh(z),
                torch.div(2, torch.exp(z) + torch.exp(-z)),
            ),
            dim=-1,
        )
...
class VarNetLogisticUnetSens(LitBaseE2E):
    """
