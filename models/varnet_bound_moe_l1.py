...
class VarNetBoundMOEL1(LitBaseE2E):
    """
    ...
    def __init__(
        ...
        self.cascades_slow = nn.ModuleList(
            [
                VarNetBlock(image_processor=make_cascade_image_processor())
                for _ in range(num_cascades)
            ]
        )
        self.cascades_fast = nn.ModuleList(
            [
                VarNetBlock(image_processor=make_cascade_image_processor())
                for _ in range(num_cascades)
            ]
        ...
        self.criterion = SSIMLossWithL1(alpha=l1_alpha)

    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        sens_maps = self.sens_net(masked_kspace, mask)
        kspace_pred = masked_kspace.clone()
        
        weight = torch.ones((1, 1, self.overlap_window), dtype=torch.uint8)
        mask_cpu = mask.clone().cpu().squeeze().unsqueeze(0)
        overlap = torch.min(F.conv1d(mask_cpu, weight))

        assert overlap.dtype == torch.uint8

        for cascade in self.cascades_slow if int(overlap) > 0 else self.cascades_fast:
