...
class UNetD(nn.Module):
    ...
    def __init__(self, in_chn, wf=32, depth=5, relu_slope=0.2, subspace_dim=16):
        ...
        for i in range(depth):
            ...
            down_path.append(UNetConvBlock(prev_channels, (2**i)*wf, downsample, relu_slope))
        ...
        for i in reversed(range(depth - 1)):
            up_path.append(UNetUpBlock(prev_channels, (2**i)*wf, relu_slope, subnet_repeat_num, subspace_dim))
            prev_channels = (2**i)*wf
            subnet_repeat_num += 1
...
class Subspace(nn.Module):
    ...
    def __init__(self, in_size, out_size):
        ...
        blocks = []
        blocks.append(UNetConvBlock(in_size, out_size, False, 0.2))
...
class skip_blocks(nn.Module):
    ...
    def __init__(self, in_size, out_size, repeat_num=1):
        ...
        blocks = []
        blocks.append(UNetConvBlock(in_size, mid_c, False, 0.2))
        for i in range(self.re_num - 2):
            blocks.append(UNetConvBlock(mid_c, mid_c, False, 0.2))
        blocks.append(UNetConvBlock(mid_c, out_size, False, 0.2))
        self.blocks = nn.ModuleList(blocks)
