...
class VarNetFull(LitBaseE2E):
    ...
    def __init__(
        ...
        self.casacde_processor = make_cascade_image_processor()
        self.cascades = nn.ModuleList(
            [
                VarNetBlock(image_processor=self.casacde_processor)
    ...
    def forward(
        ...
        nafnet_result = self.nafnet(torch.stack([varnet_result], dim=1))
        nafnet_result = self.postprocess(nafnet_result).squeeze(1) + varnet_result

        nafnet_result = self.mooosh(nafnet_result)
        nafnet_result = self.nafnet_final(torch.stack([nafnet_result], dim=1))
