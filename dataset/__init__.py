...
def collate_with_none(batch: Sequence[DataType]):
    batch_without_none = [
...
class SliceDataTransform:
    ...
    def __call__(self, mask, input, target, attrs, fname, slice) -> DataType:
...
class SliceDataModule(L.LightningDataModule):
    ...
    def __init__(
        ...
        max_key: str = "max",
        train_postfix: str = "train",
        val_postfix: str = "val",
        leaderboard_postfix: str = "leaderboard",
    ):
        super().__init__()
        ...
        public_acc, private_acc = sorted(
            os.listdir(root / leaderboard_postfix),
            key=lambda x: (x not in self.PUBLIC_ACCS),
        )

        self.path_train = root / train_postfix
        self.path_val = root / val_postfix
        self.path_test = root / leaderboard_postfix / public_acc
        self.path_predict = root / leaderboard_postfix / private_acc
    ...
    def setup(self, stage: str):
        ...
        if stage in ("predict",) and self.dataset_predict is None:
            self.dataset_predict = SliceData(
                self.path_predict,
                transform=SliceDataTransform(),
                input_key=self.input_key,
            )
