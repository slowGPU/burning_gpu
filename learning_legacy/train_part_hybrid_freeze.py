...
from utils.model.varnet_hybrid_freeze import HybridFreezeVarNet
...
def train_epoch(args, epoch, model, data_loader, optimizer, loss_type):
    ...
    for iter, data in enumerate(data_loader):
        ...
        output = model(kspace, mask, requires_sens_net_grad=True)
        loss = loss_type(output, target, maximum)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        output = model(kspace, mask, requires_sens_net_grad=False)
        loss = loss_type(output, target, maximum)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def train(args):
    ...
    model = HybridFreezeVarNet(
        num_enc_cascades=args.enc_cascades,
        num_dec_cascades=args.dec_cascades,
        num_shared_cascades=args.shared_cascades,
        chans=args.chans,
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        save_model(
            args, args.exp_dir, epoch + 1, model, optimizer, best_val_loss, is_new_best
        )
        print(
